...
export default function RownMsg({
    _id,
    members,
    loggedUser,
    setCurrentChat,
    ...
    useEffect(() => {
        ...
        const getFriend = async () => {
            try {
                const res = await axios.get(`https://snow-net.herokuapp.com/api/users/${friendId}`);
                setFriend(res.data);
                setUser(res.data);
            } catch (err) {
                console.log(err);
    ...
    return (
        <>
            {friend && (
                <div className={styles.rowContainer} onClick={handleClick} ref={ref}>
